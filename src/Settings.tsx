...
export default function Settings({ onPPKAdd, onFolderAdd }: any) {
  ...
  const folders = useLiveQuery(() => db.workspacs.toArray(), []);
  const keys = useLiveQuery(() => db.keys.toArray(), []);
  const [title, setTitle] = useState('');
  const [editBook, setBookEdit] = useState<WorkSpace>();

  function startEditTitle(item: WorkSpace) {
    if (!editBook) {
      setTitle(item.title || '桌面');
    }
    setBookEdit(item);
  }
  ...
  async function checkEditEnd(e: React.KeyboardEvent) {
    ...
    await db.updateWorkSpaceTitle(editBook!);
    setBookEdit(undefined);
  }

  return (
    <ConfigProvider renderEmpty={customizeRenderEmpty}>
      <div className="workhome-settings">
        <Scrollbars autoHide>
          <Collapse ghost>
                    ...
                    <List.Item.Meta
                      ...
                      title={
                        <span>
                          ...
                          <Popconfirm
                            title="删除后将移除桌面?"
                            onConfirm={() => db.deleteWorkSpace(item.id!)}
                            okText="删除"
                            cancelText="取消"
                          >
                            <DeleteOutlined title="删除桌面" />
                          </Popconfirm>
                        </span>
                      }
                      description={
                      ...
                      title={
                        <span>
                          ...
                          <DownloadOutlined
                            ...
                            onClick={() => {
                              download(
                                `key-workhome-${new Date().getTime()}.pem`,
                                createPem(item.pubKey, item.priKey)
                              );
                            }}
                          ></DownloadOutlined>

...
export function start() {
  ...
  liveQuery(async () => {
    return await db.cards
      .filter((card) => card.checkAt?.toString() === new Date(0).toString())
      .count();
...
async function checkSync() {
  ...
  // 30分钟内同步一次
  const lastAt = dayjs(getDateNow())
    .subtract(options?.syncMin || 10, 'minute')
    .toDate();
  // 从小到大时间顺序同步数据
  const upnotes = await db.cards
    .orderBy('checkAt')
    .filter(
      (card) =>
        !!card.wsId &&
      ...
      if (key) {
        ...
        try {
          if (!card.enabled) {
            ...
            card.hash = await uploadFileEncrypted(
              JSON.stringify({ type: card.type, props: card.props }),
              '/' + workspace.name + '/' + card.name,
              key,
              card.hash,
              card.force
  ...
  // 拉取服务器文件
  const upworkspaces = await db.workspacs
    .orderBy('checkAt')
    .filter((workspace) => !workspace.checkAt || workspace.checkAt < lastAt)
    .toArray();
    ...
    try {
      ...
      if (!workspace.enabled) {
        ...
        if (stat.cid.toString() !== workspace.hash) {
          ...
          // 删除本地
          const removes = await db.cards
            .where('wsId')
            .equals(workspace.id!)
            .filter((card) =>
              files.every(
                (file) =>
                  file.path !== '/' + workspace.name + '/' + card.name &&
                  card.syncAt &&
                  card.syncAt > card.updateAt!
              )
            );
          ...
          for (const file of files) {
            let card;
            try {
              card = await db.cards.get({
                name: file.path.split('/')[2],
                wsId: workspace.id!,
              });
              const json = JSON.parse(
                await downloadFileEncrypted(file.path, findKey)
              );
              if (!card) {
                // 新增
...
export async function deleteFile(
  ...
  await ipfs!.files.rm(ipfspath, { recursive: true });
}
...
export async function downloadFileEncrypted(
  ipfspath: string,
  findKey: (keyName: string) => Promise<Key | undefined>
) {
  try {
    ...
    const key = decryptRSA(
      buff.slice(10, 694).toString('utf8'),
      keyPair.priKey
    );
